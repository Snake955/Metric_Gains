...
console.log(
  isDemoMode
    ? "🎭 DEMO MODE: Full funksjonalitet med lokale lydfiler"
...
export async function logoutFromSpotify() {
  try {
    ...
    console.log("👋 Logged out and cleared audio");
  } catch (error) {
...
export async function playTrack(token: string, trackUri?: string) {
  if (token.startsWith("DEMO_TOKEN_")) {
    if (isLoadingSound) {
      console.log("⏳ Already loading sound...");
      return false;
    }
  ...
  if (token.startsWith("DEMO_TOKEN_")) {
    if (sound) {
      try {
        ...
        console.log("Cannot pause - sound not loaded");
      }
    }
    return false;
  }
  ...
  if (token.startsWith("DEMO_TOKEN_")) {
    if (sound) {
      try {
        ...
        console.log("Cannot resume - restarting track");
